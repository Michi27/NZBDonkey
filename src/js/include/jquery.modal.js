...
function modal(e) {
	...
	e.cModal = function(t) {
		var n = {
			...
			t = e.extend({}, n, t),
			r, i = e('<div id="nzbdonkey-modal-window" />').hide(),
			s = t._classes.box,
			o = i.append(t.template),
			u = {
				init: function() {
					e("#nzbdonkey-modal-window").remove();
					u._setStyle();
					u._modalShow();
					u._modalConent();
					i.on("click", "a.modal-btn", function(t) {
						...
						if (t.closeClick) {
							if (e.target.id == "nzbdonkey-modal-window") {
                                r = false;
								u._modalHide()
							}
						}
				...
				_modalBtn: function(n) {
					var s = false,
						...
						a = n.index(),
						f = t.buttons[a];
					if (e.inArray(o, ["confirm", "prompt"]) > -1) {
						r = s = a == 1 ? true : false;
						if (o == "prompt") {
